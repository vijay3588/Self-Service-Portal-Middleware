...
const getQuoteDetails = async (req, res) => {
  ...
  let requestBody = payloadCreator.createPayload(req, "ViewQuoteDetails", reqUserInfo);
  let requestInfo = requestCreator.createRequest("contractDetails", req.headers, true);
  ...
  if (response){
    ...
    if (userLayoutsFolder.length > 1){
        userLayoutsFolder = "../../segments/layoutsSegment" + "/" + userLayoutsFolder[1];
    }
    ListCardData = responseCreator.createResponse(response._Result, "ViewQuoteDetails", layoutObj, reqUserInfo, userLayoutsFolder);
  }
  ...
  if (response) {
    ...
    if (result.payments && result.payments.length > 0) {
      for (var i = 0; i < result.payments.length; i++) {
        ...
        var parts = {
          amountPaid: (payment.grossAmount && payment.grossAmount.amount) || "",
      ...
      viewHeader: {
        paymentNumber:
          (result.policyNumber &&
            ...
            "Contract Number " +
              result.policyNumber.internalPolicyIdentifier.internalPolicyId) ||
          "",
        paymentStatus: policyInformation.status || "",
      },
      blueBox: {
        effectiveDate: policyInformation.effectiveDate || "",
        signatureDate: policyInformation.signatureDate || "",
        termOrRenewalDate: policyInformation.termOrRenewalDate || "",
        enrollmentPeriodKind: enrollmentPeriods.enrollmentPeriodKind || "",
        endDate: enrollmentPeriods.endDate || "",
        renewalDate :    result.quoteValidEndDate || ""
      },
      disbursementCard: {
        effectiveDate: enrollmentPeriods.effectiveDate || "",
        termOrRenewalDate: policyInformation.termOrRenewalDate || "",
...
module.exports = { getQuoteDetails };
